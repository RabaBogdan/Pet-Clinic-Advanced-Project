    ...
    public static void showOption(int option) throws IOException, CsvValidationException {
        ...
        switch (option) {
            ...
            case 6:
                ...
                String[] petNames = {
                        "Fender", "Dutch", "Damia", "Mauser", "Marine",
                        "Arya", "Rowan", "Samson", "Heath", "Andra",
                        "Amber", "Arya", "Astra", "Avril", "Breanne",
                        "Briana", "Bridget", "Carla","Damia", "Harley",
                        "Jenna", "Kahlan", "Kelsey", "Kitana", "Matilda",
                        "Nina", "Rogue", "Rowan", "Roxie", "Sheba", "Storm",
                        "Trixie", "Ursa", "Valkyrie", "Zelda"};
                String[] firstNames = {
                        "John", "Davis", "Andrew", "Abraham",
                        "Alex", "Vlad", "Robert", "Simon", "Ethan"} ;
                String[] lastNames = {
                        "Davis", "Johnson", "Jhonson",
                        "Putin", "Ibrahimovich", "Lincoln"};
                String[] specialities = {
                        "Chirurg" , "Intern", "Volunteer",
                        "First Aid", "Operations", "Farm Animals"};
                String[] addresses = {
                        "2793  Old Dear Lane", "210  My Drive",
                        "4222  Chenoweth Driver", "2030  Myra Street", "3508  Tipple Road"};
                String[] races = {
                        "Affenpinscher", "Afghan Hound", "Aidi", "Airedale Terrier",
                        "Akbash", "Akita", "Alano Español", "Alapaha Blue Blood Bulldog",
                        "Alaskan husky", "Alaskan Klee Kai", "Alaskan Malamute", "Alopekis",
                        "Alpine Dachsbracke", "American Bulldog", "American Bully",
                        "American Cocker Spaniel", "American English Coonhound",
                        "American Eskimo Dog", "American Foxhound", "American Hairless Terrier",
                        "American Pit Bull Terrier", "American Staffordshire Terrier",
                        "American Water Spaniel", "Andalusian Hound", "Anglo-Français de Petite Vénerie",
                        "Appenzeller Sennenhund", "Ariegeois", "Armant", "Armenian Gampr dog", "Artois Hound"};
                for(int i = 0; i<20; i++) {
                    int phoneNumberGenerator = 100000 + random.nextInt(899999);
                    int fn1 = random.nextInt(firstNames.length), ln1 = random.nextInt(lastNames.length);
                    String ln = lastNames[ln1], fn = firstNames[fn1];
                    owner.setFirstName(firstNames[fn1]);
                    owner.setLastName(lastNames[ln1]);
                    owner.setEmail(fn+ln+"@gmail.com");
                    owner.setPhoneNumber("765"+ phoneNumberGenerator);
                    dao.createOwner(owner);

//                    vet.setFirstName(lastNames[ln1]);
//                    vet.setLastName(firstNames[fn1]);
//                    vet.setSpeciality(specialities[random.nextInt(specialities.length)]);
//                    vet.setAddress(addresses[random.nextInt(addresses.length)]);
//                    veterinarianDao.createVeterinarian(vet);

//                    pet.setName(petNames[random.nextInt(petNames.length)]);
//                    pet.setBirthDate((1 + random.nextInt(30)) +"-"+1+ random.nextInt(11)+"-"+ (2010+random.nextInt(10)));
//                    pet.setRace(races[random.nextInt(races.length)]);
//                    pet.setVaccinated(random.nextBoolean());
